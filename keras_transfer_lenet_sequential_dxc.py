from __future__ import print_function

"""03/14/2018"""
"""XD"""
"""lenet + mnist + keras"""

import datetime
import keras
from keras.datasets import mnist
from keras.models import Sequential
from keras.layers import Dense, Dropout, Activation, Flatten
from keras.layers import Conv2D, MaxPooling2D
from keras import backend as K
import matplotlib.pyplot as plt
from keras.utils import plot_model
from keras.optimizers import Adam
from keras import initializers

now = datetime.datetime.now

batch_size = 128
# num_classes = 5
epochs = 10

# input image dimensions
img_rows, img_cols = 28, 28
# number of convolutional filters to use
filters = 32
# size of pooling area for max pooling
pool_size = 2
# convolution kernel size
kernel_size = 5

if K.image_data_format() == 'channels_first':
    input_shape = (1, img_rows, img_cols)
else:
    input_shape = (img_rows, img_cols, 1)


def train_model(model, train, test, num_classes):
    x_train = train[0].reshape((train[0].shape[0],) + input_shape)
    x_test = test[0].reshape((test[0].shape[0],) + input_shape)
    x_train = x_train.astype('float32')
    x_test = x_test.astype('float32')
    x_train /= 255
    x_test /= 255
    print('x_train shape:', x_train.shape)
    print(x_train.shape[0], 'train samples')
    print(x_test.shape[0], 'test samples')

    # convert class vectors to binary class matrices
    y_train = keras.utils.to_categorical(train[1], num_classes)
    y_test = keras.utils.to_categorical(test[1], num_classes)

    model.compile(loss='categorical_crossentropy',
                  optimizer=adam,
                  metrics=['accuracy'])

    t = now()
    history = model.fit(x_train, y_train,
              batch_size=batch_size,
              epochs=epochs,
              verbose=1,
              validation_data=(x_test, y_test),
              )
    print('Training time: %s' % (now() - t))
    score = model.evaluate(x_test, y_test, verbose=0)
    print('Test score:', score[0])
    print('Test accuracy:', score[1])

    # # summarize history for loss
    # plt.plot(history.history['loss'])
    # plt.plot(history.history['val_loss'])
    # plt.title('model loss')
    # plt.ylabel('loss')
    # plt.xlabel('epoch')
    # plt.legend(['train', 'test'], loc='upper left')
    # plt.show()

    # summarize history for accuracy
    plt.plot(history.history['acc'])
    plt.plot(history.history['val_acc'])
    # plt.ylim(0.5,1.0)
    plt.title('accuracy')
    plt.ylabel('accuracy')
    plt.xlabel('epoch')
    
    

# the data, split between train and test sets
(x_train, y_train), (x_test, y_test) = mnist.load_data()

# create two datasets one with digits below 5 and one with 5 and above

num_classes = 5
num_classes2 = 5
x_train_lt5 = x_train[y_train <=4] # [0 1 2 3 4]
y_train_lt5 = y_train[y_train <=4]
x_test_lt5 = x_test[y_test <=4]
y_test_lt5 = y_test[y_test <=4]

x_train_gte5 = x_train[y_train >=5 ]
y_train_gte5 = y_train[y_train >=5 ] - 5
x_test_gte5 = x_test[y_test >= 5]
y_test_gte5 = y_test[y_test >=5 ] - 5

# define two groups of layers: feature (convolutions) and classification (dense)
feature_layers = [
    Conv2D(32, kernel_size,
           padding='same',
           input_shape=input_shape,
           kernel_initializer='random_normal'),
    Activation('relu'),
    MaxPooling2D(2, 2, 'same'),
    Conv2D(64, kernel_size,
           padding='same',
           kernel_initializer='random_normal'),
    Activation('relu'),
    MaxPooling2D(2, 2, 'same'),
    # Dropout(0.25),
    Flatten(),
    Dense(1024, kernel_initializer='random_normal'),
    Activation('relu'),
]

classification_layers1 = [    
    # Dropout(0.5),
    # Dense(1024, kernel_initializer='random_normal'),
    # Activation('relu'),
    Dense(num_classes, kernel_initializer='random_normal'),
    Activation('softmax')
]

classification_layers2 = [
    
    # Dropout(0.5),
    # Dense(1024, kernel_initializer='random_normal'),
    # Activation('relu'),
    Dense(num_classes2, kernel_initializer='random_normal'),
    Activation('softmax')
]


# create complete model
model1 = Sequential(feature_layers + classification_layers1)
adam = Adam(lr=5e-4)

# train model for 5-digit classification [0..4]
train_model(model1,
            (x_train_lt5, y_train_lt5),
            (x_test_lt5, y_test_lt5), num_classes)
plot_model(model1, to_file='model1.png')

# import scipy.io as scio
# scio.savemat('divide_dataset_mode0.mat', {'first_image':first_image, 'first_label_long':first_label_long, 'first_label_short':first_label_short,
#             'second_image':second_image, 'first_label_global':first_label_global, 'second_label_long':second_label_long, 'second_global_label':second_global_label, 'second_label_short':second_label_short})


adam = Adam(lr=1e-4)
model2 = Sequential(feature_layers + classification_layers2)
# freeze feature layers and rebuild model
for l in feature_layers:
    l.trainable = False

# transfer: train dense layers for new classification task [5..9]
train_model(model2,
            (x_train_gte5, y_train_gte5),
            (x_test_gte5, y_test_gte5), num_classes2)

plot_model(model2, to_file='model2.png')


plt.legend(['train first list', 'test first list','train second list', 'test second list'], loc='best')
plt.show()
